...
namespace Utils {
	...
	public class CMsgDlg : Form {
		...
		private static System.Drawing.Point _location;
		...
		private void AddButtons(string[] btnText) {
			...
			for (int i = btnText.Count() - 1; i >= 0; i--) {
				if (btnText[i] != "")
					AddButton(btnText[i]); // New btn to left of previous
			}
		}
		...
		public CMsgDlg ShowProgress(Form parent, string text, string caption, string btnText = "Cancel") {
			...
			// https://www.codeproject.com/Articles/1239528/A-Multipurpose-Message-Dialog?msg=5511488#xx5511488xx
			...
			if (_prgBar.Bottom > this.ClientSize.Height)
				this.Height = _prgBar.Bottom;
		...
		public string ShowDialog(string text, string caption, string[] btnText) {
			this.Text = caption;
			if (text != null)
				_lbl.Text = text; // May be null for AddTextBox()
		...
		public string ShowDialog(string text, string caption, string btnText = "OK", string btn2Text = "") {
			return ShowDialog(text, caption, new string[] { btnText, btn2Text });
		}

		public List<string> ShowDialog(string text, string caption, string[] btnText, string[] chkBoxText = null, string[] txtboxText = null) {
			this.Text = caption;
			_lbl.Text = text;

			if (chkBoxText != null) {
				foreach (string str in chkBoxText)
					AddCheckBox(str);
			}
			...
			if (btnText == null)
				btnText = new string[] { "\xdOK", "Cancel" }; // Use defaults
			AddButtons(btnText);
		...
		public List<string> ShowDialogTextBox(string caption, string lblText, string txtboxText, string[] btnText = null) {
			return ShowDialogTextBox(caption, new string[] { lblText, txtboxText }, btnText);
		}

		public List<string> ShowDialogTextBox(string caption, string[] lbl_txtboxText, string[] btnText = null) {
			...
			if (btnText == null) // Use defaults
				...
				ShowDialog(null, caption, btnText);
		...
		public TextBox AddTextBox(string lblText, string tbxText) {
			...
			if (nlines > 1) {
				...
				for (int i = 1; i < astr.Length; i++) {
					if (astr[i].Length > longstr.Length)
						longstr = astr[i];
				}
		...
		public List<string> ShowDialogCheckBox(string lblText, string caption, string[] chkBoxText, string[] btnText = null) {
			...
			if (btnText == null) // Use defaults
				...
				ShowDialog(lblText, caption, btnText);
		...
		public string Result(double perc, string text = "") {
			try {
				if (_prgBar != null) {
					if (perc <= 1.0)
						perc *= 100; // Convert to percentage
		...
		private int GetNextTopPos() {
			...
			foreach (Control ctl in _ctl.Controls) {
				if (ctl is TextBox || ctl is CheckBox) {
					if (top < ctl.Bottom + _margin)
						top = ctl.Bottom + _margin;
					if (ctl is TextBox)
						top += 2; // Allow more space
				}
			}
