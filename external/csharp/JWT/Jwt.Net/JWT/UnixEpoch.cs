...
namespace JWT
{
    public static class UnixEpoch
    {
        ...
        public static double GetSecondsSince(DateTimeOffset time) { 
        	return Math.Round((time - Value).TotalSeconds);}

        public static string GetSecondsSinceAsString(DateTimeOffset time) { return GetSecondsSince(time).ToString(CultureInfo.InvariantCulture);}
    }
}