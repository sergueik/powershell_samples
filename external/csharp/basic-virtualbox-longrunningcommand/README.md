...
> For sample mock data processing see `Test/JSONHelperTest.cs`
...
set up something for testing

```sh
docker container run -d -p 8080:8080 --entrypoint '' --name alpine1 alpine:3.18 /bin/sh -c "trap : TERM INT; (while true; do sleep 1000; done) & wait"
```
```sh
docker container run -d -p 8081:8080 --entrypoint '' --name alpine2 alpine:3.18 /bin/sh -c "trap : TERM INT; (while true; do sleep 1000; done) & wait"
```
collect running container information as JSON

> NOTE: [moby/moby#46906](https://github.com/moby/moby/issues/46906) says:
> "For compatibility reason, this can't be fixed".
>
> Multiple `--format json` records are emitted as a __JSON__ object stream rather
> than as a single __JSON__ array. `jq -s` is required to turn the stream into
> one __JSON__ array.
>
> When only a single container is running, `jq -s` still converts the single
> __JSON__ object into a __JSON__ array. This is actually quite useful for the C# processor, which cannot automatically oerce a JSON object into an array (or vice versa).
...
```json
{
  "Command": "\"/bin/sh -c 'trap : …\"",
  "CreatedAt": "2026-09-11 18:46:04 -0400 EDT",
  "ID": "7fe7800de013",
  "Image": "alpine:3.18",
  "Labels": "",
  "LocalVolumes": "0",
...
```
```sh
docker container ps --format json | jq -s | jq '.'
```
```json
[
  {
    "Command": "\"/bin/sh -c 'trap : …\"",
    "CreatedAt": "2026-09-11 19:00:24 -0400 EDT",
    "ID": "1737b1c9311e",
    "Image": "alpine:3.18",
    "Labels": "",
    "LocalVolumes": "0",
...
this can be collected by app on the Windows end:
```cmd
set PASSWORD=
set USERNAME=sergueik
set VM={7e261a39-d356-4eb1-a8ed-75675b149241}
vboxmanage.exe guestcontrol %VM% run --username %USERNAME% --password %PASSWORD% --exe /bin/sh -- /bin/sh -c "/usr/bin/docker ps --format json | jq -s | jq '.'"
```
```json
[
  {
    "Command": "\"/bin/sh -c 'trap : ΓÇª\"",
    "CreatedAt": "2026-09-11 19:00:24 -0400 EDT",
    "ID": "1737b1c9311e",
    "Image": "alpine:3.18",
    "Labels": "",
    ...
    "Networks": "bridge",
    "Platform": null,
    "Ports": "0.0.0.0:8443->8443/tcp, [::]:8443->8443/tcp, 0.0.0.0:8081->8080/tcp, [::]:8081->8080/tcp",
    "RunningFor": "33 minutes ago",
    "Size": "0B",
    "State": "running",
