...
namespace Utils
{
	...
	public class PowershellStarter
	{
		...
		public void Start(string ScriptPath, string ScriptParameters, string workingDirectory) {
			...
			PSProcess.ErrorDataReceived += (Object sender, DataReceivedEventArgs EventArgs) => collectError(EventArgs);		

			// TODO: ignore empty error ?
			bool status = PSProcess.Start();
			if (!status) {
				processError.Add("Failed to start: " + PSProcess.ExitCode);
			}
		...
		private void collectError(DataReceivedEventArgs eventArgs) {
			if (!String.IsNullOrEmpty(eventArgs.Data))
				processError.Add(eventArgs.Data); 
		}
