...
namespace Log2Console.Receiver
{
    ...
    public class WebSocketsReceiver : BaseReceiver
    {
        ...
        private void Start()
        {
            ...
            // https://learn.microsoft.com/en-us/dotnet/api/system.net.websockets.websocketstate?view=netframework-4.5
            // Cannot convert null to 'System.Net.WebSockets.WebSocketState' because it is a non-nullable value type (CS0037)
        ...
        private void OnBufferReceived(Task<WebSocketReceiveResult> obj)
        {
            if (obj.IsCompleted)
            {
                ...
                if (obj.Result.EndOfMessage)
                {
                    ...
                    logMsg.LoggerName = String.Format("{0}_{1}",loggerName,logMsg.LoggerName);
        ...
        private void NotifyWebSocketStateChange(WebSocketState state)
        {
            ...
            var logMsg = ReceiverUtils.ParseLog4JXmlLogEvent(String.Format("WebSocket state changed: {0}",state), "wssLogger");
