...
namespace Utils {
	public class JsonnetVm {
		...
		public JsonnetVm AddNativeCallback(string name, Delegate d) {
			...
			var parameterNames = parameters.Select(p => p.Name).Concat(new string[] { null }).ToArray();
			   // Append not available in C# 5
