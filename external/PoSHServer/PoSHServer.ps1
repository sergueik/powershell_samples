...
#>
  ...
  $PoSHServerPath = $env:POSHSERVER_HOME

  pushd $PoSHServerPath

  # Set PoSH Server Module Path
  $PoSHModulePath = "$PoSHServerPath\modules\PoSHServer"
  Write-Host ('$PoSHModulePath = {0}' -f $PoSHModulePath)
  ...
  if (!$PoSHModulePathTest)
  {
    Write-Warning ("Could not detect PoSH Server Module Path [{0}]." -f $PoSHModulePath)
...
if ($JobID -and $ResultCode -ne "-1")
{
  ...
  if ($JobIDPath)
  {
    ...
    $Hostname,$Port,$SSLIP,$SSLPort,$SSLName,$HomeDirectory,$LogDirectory,$CustomConfig,$CustomChildConfig,$CustomJob = $JobIDContent.Split(";")
    <#
