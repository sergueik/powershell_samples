...
namespace TalkingClipboard {
	public partial class MainFrm : Form {
		...
		private void _comboVoices_SelectionChangeCommitted (object sender, EventArgs e) {
			string voiceName = _comboVoices.SelectedItem as string;
			_synth.SelectVoice(voiceName);
		}

		private void _knobRate_ValueChanged (object sender, EventArgs e) {
		...
		private void _btnPauseRestart_Click (object sender, EventArgs e) {
			if (_synth.State == SynthesizerState.Speaking) {
				_synth.Pause();
		...
		private async void _btnSaveAsWavFile_Click (object sender, EventArgs eventArgs) {
      	
      	
			// NOTE: Windows Form apps do not have access to Environment
			// TODO: https://www.pinvoke.net/default.aspx/advapi32/CredRead.html
