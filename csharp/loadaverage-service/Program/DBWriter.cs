...
namespace TransactionService
{
	public class DBWriter : ServiceBase
	{
		...
		// NOTE: Java style variable naming
		// private string categoryName = "System";
		// NOTE: on single CPU host the counter "% Total Processor Time" will be not avauable in the category "System"
		// and only  the "% Processor Time" in category "Processor" will
		private string categoryName = "Processor";
		private string counterName = "% Processor Time";
		// TODO: specify the Instance (0)
...
//      }
		...
		protected override void OnStart(string[] args)
		{
			...
			eventLog.WriteEntry(String.Format("{0} Service Started Successfully on {1}. " + "\r\n" + "Collection Inteval: {2}. AutoAverage Interval: {3}" + "\r\n" + "DEBUG: {4}" + "\r\n" + "NOOP: {5}" + "\r\n" + "AUTORUN: {6}" + "\r\n" + "CATEGORY: {7}" + "\r\n" + "COUNTER: {8}"+ "\r\n" + "INSTANCE: {9}" , this.ServiceName, System.DateTime.Now.ToString(), collectInterval, autoAverageInterval, debug, noop, autorun, this.categoryName, this.counterName, "0" /* instanceName */), EventLogEntryType.Information);
		}
		...
		private void CollectMetrics()
		{
			...
			if (noop) {
				...
				try {
					...
					performanceCounter.InstanceName = "0";
					// For processors, use the System: %Total Processor Time counter. 
					// This averages the active time of each processor over all processor
